...
│   └── __tests__/                 # 前端测试（9 个测试文件，当前主工作区 20 个用例全部通过）
├── src-tauri/                     # 后端 Rust + Tauri
│   ├── src/
│   │   ├── main.rs                # Tauri 命令注册（26 个命令）+ 去重/转格式/断链修复命令
...
| [十一、前后端通信](#十一前后端通信tauri-ipc-机制) | Tauri IPC invoke 机制、serde camelCase 序列化、26 个命令清单 |
...
| `npm test` | 运行 Vitest 测试（9 个文件，当前主工作区 20 个用例全部通过） |
