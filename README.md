...
| [三、撤回机制](#三撤回机制如何安全地回退操作) | 按条目/按任务逆序撤回，move 可逆，delete 不可逆 |
| [四、笔记迁移](#四笔记迁移整篇笔记连带附件搬家) | 连带附件搬家，先移附件后移笔记的安全顺序 |
| [五、缩略图系统](#五缩略图系统三级缓存加速浏览) | 三级尺寸、哈希命名、一次打开多次缩放、磁盘缓存 |
| [六、虚拟滚动](#六虚拟滚动画廊万级图片不卡顿) | @tanstack/react-virtual 原理、自适应列数、overscan 预渲染 |
| [七、主题系统](#七主题系统四套主题的-css-变量方案) | CSS 变量方案、data-theme 属性切换、matchMedia 跟随系统 |
| [八、前后端通信](#八前后端通信tauri-ipc-机制) | Tauri IPC invoke 机制、serde camelCase 序列化 |
| [九、本地存储](#九本地存储替代-localstorage-的文件方案) | 替代 localStorage 的文件方案、内存缓存 + 异步写盘 |
| [十、导出报告](#十导出报告三种格式生成) | JSON/CSV/Markdown 三种格式、筛选后导出、Tauri save dialog |
| [十一、性能分析](#十一性能设计与瓶颈分析) | 当前 12 项优化措施 + 瓶颈分析 |
| [十二、Rayon 并行缩略图](#十二rayon-并行缩略图生成) | 多线程 par_iter + AtomicUsize 进度计数 |
| [十三、Tauri Event 进度流](#十三tauri-event-进度流) | 后端 window.emit → 前端 listen 实时进度推送 |
| [十四、统计图表页](#十四统计图表页recharts-可视化) | 7 种图表、CSS 变量集成、useMemo 计算 |
...
source:     原始路径
target:     目标路径
status:     Applied / Undone / Skipped / Failed
message:    可选的说明信息
```

操作历史存储在内存中的 `Mutex<Vec<OperationTask>>`（`ops_log.rs`），最多保留 200 条任务记录。
...
针对单个文件的操作进行撤回：
- **move 操作**：执行反向 `fs::rename(target, source)`，将文件从目标路径移回原始路径。如果原始目录已不存在，先 `create_dir_all` 重建。
- **delete 操作**：无法自动撤回（文件已从磁盘删除），标记为 Skipped 并提示用户。

#### 按任务撤回（`undo_task`）

对整批操作逆序撤回。关键细节是**逆序遍历**（`task.entries.iter_mut().rev()`），这样后执行的操作先撤回，避免目录依赖问题。

撤回后任务状态更新为：
- 全部条目都撤回 → `Undone`
- 部分撤回（有 delete 或失败） → `PartiallyUndone`
...
5. **记录操作**：所有移动操作记入 `OperationTask`，支持撤回

移动顺序很重要：先移附件，最后移笔记。这样如果中途出错，笔记文件仍在原位，用户可以重试。

---

### 五、缩略图系统：Rayon 并行 + WebP 格式
...
关键优化：
- **一次打开，多次缩放**：`image::open()` 是最耗时的操作（需要解码整张图片），所以即使需要生成多个尺寸，也只打开原图一次。
...
- **WebP 格式**：相比 PNG，WebP 在相同视觉质量下体积更小，大幅减少缓存磁盘占用。
...
| 缩略图缓存 | 一次打开多次缩放 + 哈希命名磁盘缓存 | 首次慢，后续秒开 |
...
- **原因**：每个 `.md` 文件都需要完整读入内存，I/O 操作是顺序的（非并行）
- **影响场景**：仓库中有数千个 Markdown 文件时，磁盘 I/O 成为瓶颈
- **可能的优化方向**：
  - 使用 `rayon` 并行读取（多线程 I/O）
  - 增量扫描（记录文件修改时间，只重新扫描变动的文件）
...
- **Phase 6 已实施的优化**：
  ...
  - ✅ 进度回调 + 前端进度条显示
- **剩余优化方向**：先从 `medium` 缩放出 `small` 和 `tiny`（级联缩放，避免三次从原图解码）
...
- **瓶颈点**：所有操作记录存储在 `Mutex<Vec<OperationTask>>` 内存中
- **原因**：当前实现不持久化到磁盘，应用重启后操作历史丢失
- **影响场景**：单次会话中执行大量修复操作后，内存占用持续增长
- **当前缓解**：限制最多保留 200 条任务记录，超出后丢弃最旧的
