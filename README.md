...
- **attachments 预处理汇总**：可在“迁移与预处理”页选择任意父级目录，提取其所有子文件夹中的 `attachments` 文件并汇总到当前目录下统一的 `attachments/`，并自动删除已空的旧 `attachments` 文件夹，方便后续扫描修复统一归位
- **Misplaced 修复复核与诊断**：批量修复错位附件后，后端会基于当前扫描规则自动复核结果，并把“仍为错位 / 变为断链 / 目标文件未被引用 / 复核通过”等结论写入操作历史，同时将结构化诊断记录追加到 `voyager-data/diagnostics/misplaced-fix.jsonl`
...
### 界面功能
- **五个 Tab 页**：附件问题扫描 / 迁移与预处理 / 统计 / 附件总览 / 说明
...
- Misplaced 修复诊断存储在 `voyager-data/diagnostics/` 中，其中 `misplaced-fix.jsonl` 为汇总日志，`misplaced-fix-{taskId}.jsonl` 为任务级诊断
...
- `npm run build`：通过（当前已完成页面级代码切分，构建阶段不再出现 Vite chunk-size warning）
- `cargo test --manifest-path src-tauri/Cargo.toml`：通过（46 个测试）
- `npm run tauri:build`：通过（当前环境已验证 Debian 12 下成功产出 `.deb` / `.rpm` / `.AppImage`；历史记录中 Windows 下也已产出 MSI 与 NSIS 安装包）

> 说明：Windows 与 Linux/Docker/WSL 之间切换开发时，`node_modules` 里的原生二进制（rollup）不兼容。切换平台后请在对应平台重新安装依赖（见下方 FAQ）。
>
> Linux 构建补充说明：`src-tauri/Cargo.toml` 现在通过 `[patch.crates-io]` 固定使用仓库内的 `src-tauri/vendor/wry/`。这是为了在 Debian 12 / WebKitGTK 环境下稳定通过 Rust/Tauri 编译与测试；不要在未验证上游修复前移除该 patch。
> 维护细节见：[`docs/maintenance/wry-linux-patch.md`](docs/maintenance/wry-linux-patch.md)
...
│   └── __tests__/                 # 前端测试（10 个测试文件，当前主工作区 21 个用例全部通过）
├── src-tauri/                     # 后端 Rust + Tauri
│   ├── src/
│   │   ├── main.rs                # Tauri 命令注册（28 个命令）+ 去重/转格式/断链修复命令
...
│   │   ├── migrate.rs             # 笔记迁移 + attachments 预处理汇总
│   │   ├── diagnostic_log.rs      # Misplaced 修复诊断 JSONL 输出
...
| [十一、前后端通信](#十一前后端通信tauri-ipc-机制) | Tauri IPC invoke 机制、serde camelCase 序列化、28 个命令清单 |
...
| [二十、操作历史持久化](#二十操作历史持久化) | JSON 文件持久化、修复诊断日志、启动加载、重启不丢失 |
...
### 六、迁移与预处理：整篇笔记迁移 + 子目录 attachments 汇总

`migrate.rs` 现在负责两类目录整理操作：

1. **联动迁移**：读取笔记内容，用 `extract_image_refs()` 提取所有图片引用，再把 `{笔记目录}/attachments/{文件名}` 移动到 `{目标目录}/attachments/{文件名}`，最后移动 `.md` 文件本身。
2. **attachments 预处理汇总**：递归查找选定目录下所有子文件夹中的 `attachments` 目录，把其中的文件统一移动到 `{所选目录}/attachments/`，然后删除那些已经搬空的旧 `attachments` 目录。
3. **冲突处理**：两类操作都共享同一套 `ConflictPolicy`（覆盖/改名/逐个确认）。
4. **记录操作**：所有移动和空目录清理操作都会记入 `OperationTask`，持久化到操作历史。

联动迁移时，移动顺序仍然是先移附件、最后移笔记；而预处理汇总则适合作为扫描修复前的一步整理，把分散的附件先聚到一个统一位置。
...
| `fix_issues` | `issues: Vec<ScanIssue>, policy?, vault_root?` | `FixSummary` | 执行修复（移动/删除/跳过 Broken），并对 Misplaced 做修复后复核 |
...
| `generate_all_thumbnails_all` | `window, paths: Vec<String>` | `ThumbGenSummary` | 批量生成附件总览页专用缩略图 |
| `get_all_thumbnail_paths` | `paths: Vec<String>` | `HashMap<String, HashMap<String, String>>` | 查询附件总览页 all-cache 缩略图路径 |
...
| `preview_flatten_attachments` | `root_dir, policy?` | `FlattenAttachmentsPlan` | 预览子目录 attachments 汇总计划 |
| `flatten_attachments` | `root_dir, policy?` | `FlattenAttachmentsSummary` | 执行子目录 attachments 汇总 |
| `open_file` | `path, vault_path` | `()` | 用系统默认程序打开 vault 内文件 |
| `open_file_parent` | `path, vault_path` | `()` | 在文件管理器中打开 vault 内文件所在目录 |
| `open_diagnostics_dir` | 无 | `String` | 打开诊断日志目录 |
| `open_misplaced_fix_diagnostic` | `task_id` | `String` | 打开指定 fix 任务的 Misplaced 诊断文件 |
| `get_runtime_logs` | `limit?` | `Vec<RuntimeLogLine>` | 获取运行日志（默认 200 条） |
| `clear_thumbnail_cache` | 无 | `CacheClearSummary` | 清除全部缩略图缓存 |
| `clear_thumbnail_cache_all` | 无 | `CacheClearSummary` | 清除附件总览页专用缩略图缓存 |
...
操作历史从纯内存存储升级为 JSON 文件持久化；Misplaced 修复还会额外输出结构化诊断日志，方便排查“点击修复但仍未修好”的情况。

- **存储路径**：`{exe_dir}/voyager-data/ops-history.json`
- **诊断路径**：`{exe_dir}/voyager-data/diagnostics/misplaced-fix.jsonl`（汇总）与 `misplaced-fix-{taskId}.jsonl`（任务级）
...
| `npm test` | 运行 Vitest 测试（10 个文件，当前主工作区 21 个用例全部通过） |
